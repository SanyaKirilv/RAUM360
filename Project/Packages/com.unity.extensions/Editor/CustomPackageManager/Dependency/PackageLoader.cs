...
public static class PackageLoader
{
    ...
    public static void Load()
    {
        ...
        foreach (PackageData package in DependencyData.dependencies)
        {
        ...
        foreach (RegistryData registry in DependencyData.scopedRegistries)
        {
            if (!manifestText.Contains($"\"name\": \"{registry.name}\""))
            {
                manifestText = InsertScopedRegistry(manifestText, registry, false);
    ...
    {
        Match match = Regex.Match(manifest, "\"dependencies\"\\s*:\\s*\\{([^}]*)\\}");
        if (match.Success)
        {
